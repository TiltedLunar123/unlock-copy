...
`yt-thumbnail.png`, 1280x720, generated by `tools/store-assets.mjs` alongside the
listing images.

Built in house rather than generated, because the subject is the product's own
UI. The screenshot in it is the real unlocked capture, so the one detail that
survives at sidebar size is a blue selection highlight on a page that was
blocking selection a moment earlier, which is the entire pitch.

Near-black rather than the brand green used on the store tiles: a thumbnail
competes in a sidebar full of bright tiles and has to win on contrast, not on
matching the rest of the listing. Checked at 246px and 168px wide, which is
